...
def load_domain_lexicon(lexicon_path="data/lexicon.txt"):
    """
    加载自定义领域词典，支持招聘实体增强
    每行一个词，例如：
        senior manager
...
def entity_postprocess(tokens, labels, lexicon):
    """
    简易后处理：若某些实体被模型漏标，但出现在词典中，可尝试修复
    """
    ...
    for term in lexicon:
        if term in sentence:
            ...
            for i in range(len(tokens) - len(term_tokens) + 1):
                if [t.lower() for t in tokens[i:i+len(term_tokens)]] == term_tokens:
                    new_labels[i] = "B-RES"
                    for j in range(1, len(term_tokens)):
                        new_labels[i + j] = "I-RES"
    return new_labels
